...
def create_window_modal():
    ...
    window = sg.Window(
        "Window Modal", layout, size=(500, 250), element_justification="c"
    )
...
def create_articles_window():
    layout = [
        ...
        [sg.Text("Sort by:"), sg.InputCombo(["Citations", "Year"], size=(40, 2))],
        [sg.Button("Submit")],
    ...
    window = sg.Window(
        "Author Articles", layout=layout, element_justification="c", size=(1024, 800)
    )
    ...
    while True:
        ...
        if event == "Submit":
            # Get author artciles
            articles = Utils.get_authors_articles_and_cites(values[0], values[1])
            # Get total citations
...
def create_window_histograms():
    ...
    window = sg.Window(
        "Author Histograms", layout=layout, element_justification="c", size=(1200, 1000)
    )
...
def create_window_cites():
    layout = [
        ...
        [
            sg.Table(
                values=[],
                headings=["Source(Journal/Conference)", "Quantity"],
                auto_size_columns=False,
                col_widths=[25, 20],
                max_col_width=80,
                display_row_numbers=True,
    ...
    window = sg.Window(
        "Citings of Papers", layout=layout, element_justification="c", size=(1200, 1000)
    )
    ...
    while True:
        ...
        if event == "Submit":
            ...
            (hist_year, hist_pub, hist_jour, data) = Utils.get_selected_papers_citings(
                selected_papers, values[0]
            )
            """
...
def create_window_all_info():
    ...
    window = sg.Window(
        "All info on papers", layout=layout, element_justification="c", size=(1024, 780)
    )
