...
class InvertedIndex:
    ...
    def add_document(self, document):
        ...
        if doc_id in self.docs:
            raise Exception(
                "Document [" + doc_id + "] already exists in the Inverted Index."
            )
    ...
    def generate_term_by_doc_matrix(self, tfidf=False, log_entropy=False):
        ...
        if tfidf and log_entropy:
            raise Exception(
                "[generate_term_by_doc_matrix]:  Error, both tfidf and log_entropy can't both be True."
            )
    ...
    def create_query_vector(self, q, tfidf=False, log_entropy=False):
        ...
        if tfidf and log_entropy:
            raise Exception(
                "[create_query_vector]:  Error, both tfidf and log_entropy can't both be True."
            )
        ...
        # Log entropy for the query vector
